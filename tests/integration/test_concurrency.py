...
@agent(name="counter")
async def counter_agent(state: ConcurrencyState) -> dict:
...
async def test_concurrent_runs_no_state_corruption():
    ...
    g.then(counter_agent, name="c1").then(counter_agent, name="c2")
    compiled_graph = g.compile()

    # Act: Run the compiled graph 5 times concurrently
    num_concurrent_runs = 5
    tasks = [run(compiled_graph, input={}) for _ in range(num_concurrent_runs)]
